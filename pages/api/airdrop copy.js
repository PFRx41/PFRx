...
export default async function handler(req, res) {
  ...
  async function logToFile(message, logFile = "/tmp/airdrop_log.txt") {
  ...
  try {
    ...
    const outputContent = [
      ...
      `Qualified Holders (on mainnet): ${qualifiedHolders.length}`,
      "-".repeat(50),
      ...holders.map(
    ...
    await fs.writeFile("/tmp/token_holders.txt", outputContent);
    await logToFile(`Saved ${holders.length} token holders (${qualifiedHolders.length} qualified) to token_holders.txt`);
    ...
    if (balance > MINIMUM_BALANCE) {
      ...
      const distributionContent = [
        "\nAirdrop Distribution (on mainnet, 10% fee wallet, 90% holders with logarithmic weighting)",
        "-".repeat(50),
      ...
      await fs.appendFile("/tmp/token_holders.txt", distributionContent);
      ...
      await logToFile("Mainnet wallet balance <= 0.2 SOL, skipping airdrop");
    }
